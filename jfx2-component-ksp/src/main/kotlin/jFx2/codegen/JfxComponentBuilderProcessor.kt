...
class JfxComponentBuilderProcessor(
    ...
    private fun generateBuilderFor(klass: KSClassDeclaration): Boolean {
        ...
        val typeParamNames = klass.typeParameters.map { it.name.asString() }
        val classTypeArgs = if (typeParamNames.isEmpty()) "" else {
            typeParamNames.joinToString(prefix = "<", postfix = ">")
        }
        val renderedComponentType = className + classTypeArgs
        ...
        OutputStreamWriter(file, Charsets.UTF_8).use { out ->
            ...
            val typeParams = renderFunctionTypeParams(klass)
            val funTypeParamsDecl = typeParams.decl?.let { " $it" }.orEmpty()

            out.append("fun$funTypeParamsDecl ${escapeKotlinIdentifier(builderName)}(")

            val params = ArrayList<String>(builderParams.size + 1)
            params.addAll(builderParams)
            params += "block: context(jFx2.core.capabilities.NodeScope) $renderedComponentType.() -> Unit = {}"
            ...
            if (typeParams.where.isNullOrBlank()) {
                out.appendLine(": $renderedComponentType =")
                out.appendLine("    jFx2.core.codegen.buildComponent(")
            } else {
                out.appendLine(": $renderedComponentType")
                out.appendLine(typeParams.where)
                out.appendLine("    = jFx2.core.codegen.buildComponent(")
            }
            ...
            out.appendLine("        create = { el: $nodeTypeRendered -> $renderedComponentType(${callArgs.joinToString(", ")}) },")
    ...
    private fun renderType(type: KSType): String {
        ...
        val base = when (decl) {
            is KSTypeParameter -> decl.simpleName.asString()
    ...
    private data class TypeParamsRendered(
        val decl: String?,
        val where: String?
    )

    private fun renderFunctionTypeParams(klass: KSClassDeclaration): TypeParamsRendered {
        val typeParams = klass.typeParameters
        if (typeParams.isEmpty()) return TypeParamsRendered(decl = null, where = null)

        val decls = ArrayList<String>(typeParams.size)
        val whereConstraints = ArrayList<String>()

        for (tp in typeParams) {
            val name = tp.name.asString()
            val bounds = tp.bounds.map { it.resolve() }.toList()
            if (bounds.isEmpty()) {
                decls += name
                continue
            }

            decls += "$name : ${renderType(bounds.first())}"
            for (b in bounds.drop(1)) {
                whereConstraints += "$name : ${renderType(b)}"
            }
        }

        val decl = decls.joinToString(prefix = "<", postfix = ">")
        val where = whereConstraints.takeIf { it.isNotEmpty() }?.joinToString(
            prefix = "where ",
            separator = ", "
        )

        return TypeParamsRendered(decl = decl, where = where)
    }
